...
screen = turtle.Screen() # Setup the screen
screen.title("Turtle Collection Game")
screen.setup(width=800, height=600)
screen.bgcolor("lightblue")

player = turtle.Turtle() # Create player turtle
player.shape("turtle")
player.color("green")
player.penup()
player.speed(10)  # Slow animation speed to see the movement

point = turtle.Turtle() # Create point turtle
point.shape("square")
point.color("gold")
point.penup()
point.speed(0)
...
obstacles = [] # Create obstacle turtles
num_obstacles = 10
...
screen.listen() # Keyboard bindings
...
while True:
    ...
    if check_collision(player, point): # Check collision with point
        update_score()
        place_point()
