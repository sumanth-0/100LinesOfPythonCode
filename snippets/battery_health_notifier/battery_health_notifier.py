#!/usr/bin/env python3
"""
Battery Health Notifier - Monitors battery and sends notifications
...
def send_notification(title, message):
    ...
    if NOTIFICATIONS_AVAILABLE:
        try:
            notification.notify(title=title, message=message, app_name='Battery Health', timeout=10)
...
def monitor_battery(check_interval=60, low_threshold=20, high_threshold=80):
    ...
    print(f"Low: {low_threshold}%, High: {high_threshold}%, Interval: {check_interval}s\n")
    
    last_status = {'low_notified': False, 'high_notified': False}
    ...
    while True:
        try:
            ...
            print(f"Battery: {percent}% | {status} | Time: {time_str}")
            
            # Low battery warning
            if not plugged and percent <= low_threshold and not last_status['low_notified']:
                send_notification("Low Battery", f"Battery at {percent}%. Please charge soon.")
